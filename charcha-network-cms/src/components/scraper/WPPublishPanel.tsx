...
export function WPPublishPanel({ articleId, rewrite, article, wpPostId, onPublish, onReject, onArticleUpdated }: WPPublishPanelProps) {
  ...
  }, [wpUrl, effectiveWpUrl, effectiveSiteId, apis.wpMeta])
  ...
  const handleUploadToWP = async () => {
    ...
    try {
      const result = await apis.wpMeta.uploadImage(fileToUpload, effectiveSiteId || undefined)
  ...
  const handleExtractImage = async () => {
    ...
    try {
      await apis.inbox.extractImage(articleId)
  ...
  const handleUseSourceImage = async () => {
    ...
    setUploadingImage(true)
    try {
      const result = await apis.publish.uploadSourceImage(articleId, wpSites.length > 1 ? effectiveSiteId : undefined)
      setUploadedMediaId(result.id)
      setUploadedMediaUrl(result.url)
