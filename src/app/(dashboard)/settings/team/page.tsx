...
import { settingsApi, teamApi, type AuthUserDto } from '@/lib/api'
import { buildRegisterInviteUrl, normalizeRegisterInviteUrl } from '@/lib/pathname'
...
function formatAccess(access?: ('studio' | 'cms')[]) {
  if (!access?.length) return '-'
  return access.map(a => (a === 'studio' ? 'Studio' : 'CMS')).join(' + ')
}
...
export default function TeamPage() {
  ...
  const [teamUsers, setTeamUsers] = useState<AuthUserDto[]>([])
  ...
  const handleInvite = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const link = normalizeRegisterInviteUrl(inviteLink)
  ...
  const handleRemoveUser = async (id: string) => {
    if (!confirm('Remove this user from the team? They will lose access.')) return
    setRemovingId(id)
    try {
      await teamApi.deleteUser(id)
  ...
  const copyInviteLink = (token: string) => {
    ...
    const link = buildRegisterInviteUrl(base, token)
  ...
  if (!hasPermission(user?.role, 'studio', 'team')) {
    return (
      <div className="flex flex-1 flex-col items-center justify-center gap-3 p-6" style={{ background: 'var(--bg)' }}>
        <p style={{ fontSize: 14, color: 'var(--text-muted)', textAlign: 'center', maxWidth: 360 }}>
          You don&apos;t have permission to manage team settings. Ask a super admin for access.
        </p>
      </div>
    )
  }
                    ...
                    {user?.role === 'super_admin' && u.id !== user?.id && (
                      <button
                        ...
                        style={{
                          ...
                          cursor: removingId === u.id ? 'not-allowed' : 'pointer',
                          opacity: removingId === u.id ? 0.6 : 1,
                        }}
                      >
                        {removingId === u.id ? (
            ...
            {invites.length > 0 && (
              <div style={{ marginTop: 16 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {invites.map(inv => {
                    const base = typeof window !== 'undefined' ? window.location.origin : ''
                    const inviteLink = buildRegisterInviteUrl(base, inv.token)
                    return (
                    <div
                      key={inv.token}
                      style={{
