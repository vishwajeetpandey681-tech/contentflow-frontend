...
export function getApiErrorMessage(err: unknown): string {
  ...
  if (ax.code === 'ERR_NETWORK' || !ax.response) {
    return 'Cannot reach the Charcha API. Start contentflow-backend (npm run dev in contentflow-backend/) and ensure NEXT_PUBLIC_CHARCHA_API_URL in .env.local points to http://localhost:4500/api.'
  }
...
export const teamApi = {
  ...
  updateUser: (id: string, body: { name?: string; role?: string; access?: string[] }) =>
